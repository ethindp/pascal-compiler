 ...
 * required: Option must be set on the command line. Otherwise an exception will
 * be thrown optional: Option must not be set. Default attribute. advanced:
 * Option is advanced and will only show up in the advanced help message
...
class Option {
  ...
  public:
    /// Construct an Option
    /// @param short_name the options's short name. Must be empty or one
    /// character.
    /// @param long_name the option's long name. Can be empty.
    /// @param description the Option's description that will be shown in the
    /// help message
    ...
    /// @return character of the options's short name or 0 if no short name is
    /// defined
    char short_name() const;

    /// Get the Option's long name
    /// @return the long name of the Option. Empty string if no long name is
    /// defined
...
template <class T> class Value : public Option {
  public:
    ...
    /// @param short_name the option's short name. Must be empty or one
    /// character.
    /// @param long_name the option's long name. Can be empty.
    /// @param description the Option's description that will be shown in the
    /// help message
    ...
    /// @param assign_to pointer to a variable to assign the parsed command line
    /// value to
    Value(const std::string &short_name, const std::string &long_name,
          const std::string &description, const T &default_val,
          T *assign_to = nullptr);
    ...
    /// Get the Option's value. Will throw if option at index idx is not
    /// available
    ...
    /// @param idx the zero based index of the value (if set multiple times)
    /// @return the Option's value at index "idx" or the default value or
    /// default_value
 ...
 * "add<Option_Type>(option params)" Call "parse(argc, argv)" to trigger parsing
 * of the options and to fill "non_option_args" and "unknown_options"
 */
class OptionParser {
  public:
    ...
    /// Add an Option e.g. 'add<Value<int>>("i", "int", "description for the -i
    /// option")'
    /// @param T the option type (Value, Switch, Implicit)
    /// @param attribute the Option's attribute (inactive, hidden, required,
    /// optional, ...)
    ...
    /// @param max_attribute show options up to this level (optional, advanced,
    /// expert)
    /// @return the help message
    std::string
    help(const Attribute &max_attribute = Attribute::optional) const;
    ...
    /// e.g. "-i 5 hello" => hello
    /// e.g. "-i 5 -- from here non option args" => "from", "here", "non",
    /// "option", "args"
    ...
    /// @return a pointer of type "Value, Switch, Implicit" to the Option or
    /// nullptr
    template <typename T>
...
class invalid_option : public std::invalid_argument {
  public:
    ...
    invalid_option(const Option *option, invalid_option::Error error,
                   OptionName what_name, std::string value,
                   const std::string &text)
        : std::invalid_argument(text.c_str()), option_(option), error_(error),
          what_name_(what_name), value_(std::move(value)) {}
...
class OptionPrinter {
  public:
    ...
    /// @return the help message
    virtual std::string
    print(const Attribute &max_attribute = Attribute::optional) const = 0;
...
class ConsoleOptionPrinter : public OptionPrinter {
  public:
    ...
    std::string
    print(const Attribute &max_attribute = Attribute::optional) const override;
...
/**
 * Creates a script with all options (short and long) that can be used for bash
 * completion
 */
class BashCompletionOptionPrinter : public OptionPrinter {
  public:
    BashCompletionOptionPrinter(const OptionParser *option_parser,
...
inline Option::Option(const std::string &short_name,
                      const std::string &long_name, std::string description)
    : short_name_(short_name), long_name_(long_name),
      description_(std::move(description)), attribute_(Attribute::optional) {
    if (short_name.size() > 1)
        throw std::invalid_argument("length of short name must be <= 1: '" +
                                    short_name + "'");
...
inline std::string Option::name(OptionName what_name, bool with_hypen) const {
    if (what_name == OptionName::short_name)
        return short_name_.empty() ? ""
                                   : ((with_hypen ? "-" : "") + short_name_);
    if (what_name == OptionName::long_name)
        return long_name_.empty() ? ""
                                  : ((with_hypen ? "--" : "") + long_name_);
    return "";
}
...
template <class T>
inline Value<T>::Value(const std::string &short_name,
...
template <class T> inline size_t Value<T>::count() const {
    return values_.size();
}

template <class T> inline bool Value<T>::is_set() const {
    return !values_.empty();
}

template <class T> inline void Value<T>::assign_to(T *var) {
    assign_to_ = var;
    update_reference();
}

template <class T> inline void Value<T>::set_value(const T &value) {
    clear();
    add_value(value);
}
...
template <class T> inline T Value<T>::value(size_t idx) const {
...
template <class T> inline void Value<T>::set_default(const T &value) {
    this->default_.reset(new T);
    *this->default_ = value;
    update_reference();
}

template <class T> inline bool Value<T>::has_default() const {
    return (this->default_ != nullptr);
}

template <class T> inline T Value<T>::get_default() const {
    if (!has_default())
        throw std::runtime_error("no default value set");
    return *this->default_;
}

template <class T> inline bool Value<T>::get_default(std::ostream &out) const {
    if (!has_default())
        return false;
    out << *this->default_;
    return true;
...
template <class T> inline Argument Value<T>::argument_type() const {
...
inline void Value<std::string>::parse(OptionName what_name, const char *value) {
    if (strlen(value) == 0)
        throw invalid_option(this, invalid_option::Error::missing_argument,
                             what_name, value,
...
inline void Value<T>::parse(OptionName what_name, const char *value) {
    ...
    if (is.fail())
        throw invalid_option(this, invalid_option::Error::invalid_argument,
                             what_name, value,
                             "invalid argument for " + name(what_name, true) +
                                 ": '" + strValue + "'");

    if (valuesRead > 1)
        throw invalid_option(this, invalid_option::Error::too_many_arguments,
                             what_name, value,
                             "too many arguments for " + name(what_name, true) +
                                 ": '" + strValue + "'");
...
template <class T> inline void Value<T>::update_reference() {
    if (this->assign_to_) {
        if (!this->is_set() && default_)
...
template <class T> inline void Value<T>::add_value(const T &value) {
    values_.push_back(value);
    update_reference();
}

template <class T> inline void Value<T>::clear() {
    values_.clear();
    update_reference();
}
...
template <class T>
inline Implicit<T>::Implicit(const std::string &short_name,
                             const std::string &long_name,
                             const std::string &description,
                             const T &implicit_val, T *assign_to)
    : Value<T>(short_name, long_name, description, implicit_val, assign_to) {}

template <class T> inline Argument Implicit<T>::argument_type() const {
...
inline Switch::Switch(const std::string &short_name,
                      const std::string &long_name,
...
inline std::shared_ptr<T> OptionParser::add(Ts &&...params) {
    ...
    std::shared_ptr<T> option =
        std::make_shared<T>(std::forward<Ts>(params)...);

    for (const auto &o : options_) {
        if ((option->short_name() != 0) &&
            (option->short_name() == o->short_name()))
            throw std::invalid_argument("duplicate short option name '-" +
                                        std::string(1, option->short_name()) +
                                        "'");
        if (!option->long_name().empty() &&
            (option->long_name() == (o->long_name())))
            throw std::invalid_argument("duplicate long option name '--" +
                                        option->long_name() + "'");
    }
...
inline Option_ptr
OptionParser::find_option(const std::string &long_name) const {
...
OptionParser::get_option(const std::string &long_name) const {
...
inline std::shared_ptr<T> OptionParser::get_option(char short_name) const {
    ...
    auto result = std::dynamic_pointer_cast<T>(option);
    if (!result)
        throw std::invalid_argument("cannot cast option to T: " +
                                    std::string(1, short_name));
    return result;
}
...
inline void OptionParser::parse(const std::string &ini_filename) {
    ...
    auto split =
        ...
        return {trim_copy(s.substr(0, pos)),
                trim_copy(s.substr(pos + 1, std::string::npos))};
    };
...
inline ConsoleOptionPrinter::ConsoleOptionPrinter(
...
inline std::string
ConsoleOptionPrinter::print(const Attribute &max_attribute) const {
    ...
    for (const auto &option : option_parser_->options())
        optionRightMargin =
            std::max(optionRightMargin, to_string(option).size() + 2);
    optionRightMargin =
        std::min(maxDescriptionLeftMargin - 2, optionRightMargin);
...
inline std::string
GroffOptionPrinter::print(const Attribute &max_attribute) const {
    if (option_parser_ == nullptr)
        return "";
...
inline std::string
BashCompletionOptionPrinter::print(const Attribute & /*max_attribute*/) const {
    if (option_parser_ == nullptr)
        return "";
